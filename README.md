...
│   ├── toolDefinitions.js схемы инструментов (OpenAI function-calling)
│   └── prompts/
...
Ядро спроектировано как шаблон. Четыре точки расширения:

1. **Поведение / личность бота** → отредактируйте `src/agent/prompts/system_prompt.txt`.
2. **Новый инструмент** → добавьте схему в `src/agent/toolDefinitions.js` + `case` в `executeToolCall()` (`src/agent/agent.js`), при необходимости файл в `src/tools/`.
...
### Пример: добавить инструмент

```js
// 1) toolDefinitions.js — схема
{
  type: 'function',
  function: {
    name: 'get_weather',
...
// 2) agent.js → executeToolCall() — обработчик
case 'get_weather': {
