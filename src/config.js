...
// PAIR_TOKEN защищает страницу привязки WhatsApp /pair. Если он не задан в env —
// генерируем случайный на старте, чтобы страница работала «из коробки» (ссылка
// печатается в логах при запуске). Для прода лучше зафиксировать свой в .env,
// иначе токен (и ссылка) меняются при каждом рестарте.
const PAIR_TOKEN = process.env.PAIR_TOKEN || crypto.randomBytes(16).toString('hex');
const PAIR_TOKEN_AUTOGENERATED = !process.env.PAIR_TOKEN;

module.exports = {
  PAIR_TOKEN_AUTOGENERATED,
  // Server
  ...
  // Remote pairing web page (/pair) — публичная страница для удалённой привязки
  // WhatsApp. PAIR_TOKEN защищает её; если не задан в env — генерируется
  // автоматически на старте (см. выше). ENABLE_TUNNEL поднимает Cloudflare Quick
