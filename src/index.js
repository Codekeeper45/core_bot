...
async function startServer() {
  ...
  const server = app.listen(config.PORT, () => {
    ...
    // Ссылка на страницу привязки WhatsApp. PAIR_TOKEN всегда есть (задан в env
    // или сгенерирован автоматически в config.js), поэтому ссылка печатается
    // при каждом старте — её можно сразу отправить тому, кто привязывает номер.
    const pairPath = `/pair?token=${encodeURIComponent(config.PAIR_TOKEN)}`;
    console.log('');
    console.log('═══════════════════════════════════════════════════');
    console.log('[Pair] Страница привязки WhatsApp:');
    console.log(`  Локально:  http://localhost:${config.PORT}${pairPath}`);
    if (config.PAIR_TOKEN_AUTOGENERATED) {
      console.log('  PAIR_TOKEN сгенерирован автоматически (меняется при рестарте).');
      console.log('  Чтобы зафиксировать — задайте PAIR_TOKEN в .env.');
    }
    if (config.ENABLE_TUNNEL) {
      console.log('  Публичная HTTPS-ссылка появится ниже, когда поднимется туннель.');
    } else {
      console.log('  Для удалённого доступа включите ENABLE_TUNNEL=1 (Cloudflare HTTPS).');
    }
    console.log('═══════════════════════════════════════════════════');
    console.log('');
