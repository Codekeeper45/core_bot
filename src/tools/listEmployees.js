...
const definition = {
  ...
  function: {
    name: 'list_employees',
    description:
      'Возвращает реестр доступных сотрудников с их ролями, навыками, ТЕЛЕФОНОМ (phone) и текущей '
      + 'нагрузкой: open_task_count + open_tasks (id, название, статус каждой открытой задачи). База '
      + 'ОБЩАЯ для всех боссов — всегда вызывай это, чтобы свериться, кто есть в штате (не полагайся '
      + 'на память). Если просят показать контакты — выводи имя, роль и phone. Вызывай это '
...
async function handler() {
  ...
  return {
    employees: rows.map((e) => ({
      ...
      roles: e.roles,
      skills: e.skills || '',
      phone: e.contact || null, // номер (WhatsApp) — чтобы босс видел/мог обновить контакт
      is_test: !e.contact,
