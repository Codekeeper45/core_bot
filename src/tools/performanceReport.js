...
// слабые места + готовая mermaid-диаграмма (для render_diagram). Доступен всем
// (read-only, иерархии нет). Все вычисления (границы месяца, проценты, выводы)
// делает код — LLM только пересказывает.
...
const definition = {
  ...
  function: {
    ...
    description:
      'Отчёт успеваемости команды за месяц (доступен всем): по каждому сотруднику — сколько задач '
