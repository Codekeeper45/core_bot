...
function startTunnel() {
  ...
  t.once('url', (url) => {
    currentUrl = url;
    // PAIR_TOKEN всегда есть (задан в env или автогенерирован в config.js).
    const pairUrl = `${url}/pair?token=${encodeURIComponent(config.PAIR_TOKEN)}`;
    console.log(`[Tunnel] ✅ HTTPS: ${url}`);
    console.log(`[Tunnel] 🔗 PAIR PAGE (отправьте тому, кто привязывает WhatsApp): ${pairUrl}`);
    if (config.PAIR_TOKEN_AUTOGENERATED) {
      console.warn('[Tunnel] ⚠️ PAIR_TOKEN автогенерирован — ссылка изменится при рестарте. Зафиксируйте PAIR_TOKEN в .env для постоянной ссылки.');
    }
